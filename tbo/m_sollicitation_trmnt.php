...
$req_tache = $db->prepare("UPDATE `sollicitation` SET `type`=?, `id_client`=?, `date_reception`=?, `num_dossier`=?, `date_limit`=?, `departement`=?, `details_ao`=? WHERE id=?");
$result = $req_tache->execute(array($type_tache, $client, $date_reception, $num_dossier, $date_limite, $list_departement, $details_ao, $id)) or die(print_r($req_tache->errorInfo()));
