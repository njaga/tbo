...
$req_tache = $db->prepare("UPDATE `tache` SET `id_client`=?, `type_tache`=?, `date_tache`=?, `commentaire`=?, `num_devis`=?, `montant`=?, `id_user`=? WHERE id=?");
$result = $req_tache->execute(array($client, $type_tache, $date_tache, $commentaire, $num_devis, $montant, $_SESSION['id_vigilus_user'], $id)) or die(print_r($req_tache->errorInfo()));
