...
$req_vente_annuel = $db->prepare("SELECT MONTH(date_validation), SUM(montant) FROM `tache` WHERE date_validation is NOT NULL AND type_tache='Devis' GROUP BY date_validation");
$req_vente_annuel->execute(array($id));
$donnees_annuel = $req_vente_annuel->fetch();


$req_vente = $db->prepare("SELECT SUM(montant) 
FROM `tache` 
WHERE id_user=? AND date_validation is NOT NULL  AND type_tache='Devis'");
...
FROM `tache` 
WHERE id_user=? AND date_validation is NULL  AND type_tache='Devis'");
