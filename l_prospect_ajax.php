...
FROM `client` 
WHERE client.etat=1 AND client.type='Prospect'");
$records = $stmt->fetch();
...
FROM `client` 
WHERE client.etat=1 AND client.type='Prospect' " . $searchQuery);
...
FROM `client`
INNER JOIN user ON user.id=client.id_user WHERE client.etat=1 AND client.type='Prospect' " . $searchQuery . " ORDER BY " . $columnName . " " . $columnSortOrder . " LIMIT :limit,:offset");
