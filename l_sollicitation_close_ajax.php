...
$mois = $_POST['mois'];
$annee = $_POST['annee'];
$id_commercial = $_SESSION['id_vigilus_user'];
...
if ($search == "") {
    ...
    FROM `sollicitation`
    INNER JOIN client ON client.id=sollicitation.id_client
    WHERE sollicitation.etat>2 AND CONCAT (client.client,' ',sollicitation.departement, ' ',sollicitation.details_ao) like CONCAT('%', ?, '%')  " . $id_commercials . " AND MONTH(sollicitation.date_reception)=? AND YEAR(sollicitation.date_reception)=? ORDER BY sollicitation.date_depot DESC");
